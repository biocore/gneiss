...
from util import match, match_tips
...
def _process(table, metadata, tree):
    ...
    non_tips_no_name = [(n.name is None) for n in tree.levelorder() if n.is_tip()]
...
def ols(formula, table, metadata, tree, **kwargs):
    ...
    metadata: pd.DataFrame
        ...
        in the `table` object.  Samples correspond to rows and covariates correspond
        to columns.
    tree : skbio.TreeNode
    ...
    **kwargs : dict
    ...
    basis, _ = balance_basis(_table, _tree)
...
def gls(formula, table, metadata, tree, **kwargs):
    """ Generalized Least Squares applied to balances.
