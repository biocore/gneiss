...
class TestOLS_Summary(unittest.TestCase):
    ...
    def test_visualization(self):
        ...
        exp_pred = {'y0': {0: -0.53375121547306381,
        ...
        self.assertDictEqual(exp_pred, pred.to_dict())

        exp_coefs = {'Intercept': {'y0': 6880999561.7020159,
        ...
        self.assertDictEqual(exp_coefs, coefs.to_dict())

        exp_resid = {'y0': {0: -0.05693401912370244,
        ...
        self.assertDictEqual(exp_coefs, resid.to_dict())

        pvals = {'Intercept': {'y0': 0.3193097383026624,
        ...
        with open(index_fp, 'r') as fh:
            html = fh.read()
            self.assertIn('<th>Simplicial</th>', html)
            self.assertIn('<th>Relative importance</th>', html)
            self.assertIn('<th>Explained Sum of Squares</th>', html)
            self.assertIn('<th>Prediction and Residual plot</th>', html)
