gneiss documentation
========================

This guide contains instructions for building the gneiss documentation, as
well as guidelines for contributing to the documentation.

Building the documentation
--------------------------

To build the documentation, you'll need a gneiss development environment
...
gneiss is *currently installed* on your system (i.e., the version imported
by ```import gneiss```). This may not match the code located in this repository.
You will need to either install this version of gneiss somewhere (e.g., in
a virtualenv) or point your ```PYTHONPATH``` environment variable to this code,
*before* building the documentation.

To build the documentation, assuming you are at the top-level gneiss
directory:
...
review our [gneiss contribution guide](../CONTRIBUTING.md).
...
Most of gneiss's API documentation is automatically generated from
...
gneiss docstrings follow the [numpydoc conventions](https://github.com/numpy/numpy/blob/master/doc/HOWTO_DOCUMENT.rst.txt).
...
### Documenting a module in gneiss
...
docstrings in existing gneiss modules. An example of a module to start with
is ```gneiss.balances```. Go ahead and look
...
you'd like to add to gneiss (let's call it ```gneiss/example.py```).
...
module:
    ...
    """
    Documentation examples (:mod:`gneiss.example`)
    =============================================
...
We also need to include another Sphinx directive below this:
    ...
    .. currentmodule:: gneiss.example

This directive tells Sphinx that other classes, functions, etc. that we will
reference are located in the ```gneiss.example``` module.
...
Next, include a more detailed description of the module. For example:
    ...
    the gneiss documentation system.
...
that they can be automatically tested (e.g., using ```make test```).
    ...
    >>> from gneiss.example import example_function1
...
1. Make sure you're within the ```gneiss/doc``` directory.
2. Create a new file with the same name as your module under the ```source```
   directory. Do not include ```gneiss``` as part of the name, and use
...
### Documenting a subpackage in gneiss

The process of documenting a subpackage is very similar to documenting a module
in gneiss. The only difference is that the module docstring goes in the
